...
//附加 js 路径
define('B_JS_PATH', pc_base::load_config('system', 'b_js_path'));
//附加 css 路径
define('B_CSS_PATH', pc_base::load_config('system', 'b_css_path'));
//附加 img 路径
define('B_IMG_PATH', pc_base::load_config('system', 'b_img_path'));

//分页大小
define('PAGESIZE', pc_base::load_config('system', 'pagesize'));

//应用静态文件路径
define('PLUGIN_STATICS_PATH', WEB_PATH . 'statics/plugin/');

//加载RSA密钥
define('PRIKEY', pc_base::load_config('system', 'prikey'));
//加载RSA公钥
define('PUBKEY', pc_base::load_config('system', 'pubkey'));
...
public static function load_app_class($classname, $m = '', $initialize = 1) {
    ...
    public static function load_app_class($classname, $m = '', $initialize = 1)
    {
        $m = empty($m) && defined('ROUTE_M') ? ROUTE_M : $m;
        if (empty($m)) return false;
        //modify 修改使模块类支持按目录存放
        if ($i = strpos($m, '/')) {
            $p = DIRECTORY_SEPARATOR . substr($m, $i + 1);
            $m = substr($m, 0, $i);
        } else {
            $p = '';
        }
        return self::_load_class($classname, 'modules' . DIRECTORY_SEPARATOR . $m . DIRECTORY_SEPARATOR . 'classes' . $p, $initialize);
    }
    ...
    private static function _load_class($classname, $path = '', $initialize = 1)
    {
        ...
        if (isset($classes[$key])) {
            if ($classes[$key] === true && $initialize == 1) {
                $name = $classname;
