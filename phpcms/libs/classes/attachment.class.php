...
class attachment {
	 ...
	 * 上传导入excel
	 */
	function upload_excel($field = 'excel', $alowexts = '')
	{
		...
		if (!$alowexts) {
			$alowexts = $this->excelexts;
		}
	
		$alowexts = implode('|', $alowexts);
	
		$fileext = fileext($file['name']); //文件扩展名
	
		if (!preg_match("/^(" . $alowexts . ")$/", $fileext)) {
			//图片格式错误
			$this->error = "excel格式错误";
			return false;
		}
		...
		if (move_uploaded_file($file['tmp_name'], $savefile)) {
			...
			$uploadedfile = array('filename' => $file['name'], 'filepath' => $filepath, 'filesize' => $file['size'], 'fileext' => $fileext, 'fn' => $file['fn'], 'isimport' => 1);
			$aid = $this->add($uploadedfile);
			$return_arr = array('aid' => $aid, 'filepath' => $savefile);
			return $return_arr; //返回保存绝对路径
		} else {
			return false;
		}
	}
