...
*/
function remove_xss_array(&$arr, $safe=''){
    if(!$safe){
        //系统参数默认为安全参数
        $safe = array('m','c','a');
    }
    if (is_array($arr)){
        foreach ($arr as $k=>$v){
            if(!in_array($k,$safe)){
                $arr[$k] = remove_xss_array($v);
            }
        }
        return $arr;
    } elseif(is_string($arr)){
        return remove_xss($arr);
    }    
}
/**
 * xss过滤函数
...
function get_address_senior_search($areaid, $id = 'villagename', $linkageid = 3360, $level = 3) {
    ...
    foreach ($infos as $val) {
        if ($val['linkageid'] == $areaid) {
            $areaid_name = $val['name'];
        }
    }
    //获取地址联动菜单级联id
    if ($areaid) {
        $areaid_array = menu_linkage_level_linkageid($areaid, $infos);
        $areaid_array[] = '3360';
    } else {
        $areaid_array[] = $linkageid;
        $areaid_array[] = '3360';
    }

    $areaid_array = array_reverse($areaid_array); //逆序
    $string = '<input type="hidden" name="' . $id . '"  id="' . $id . '" value="' . $areaid . ';' . $areaid_name . '">';
    for ($i = 0; $i <= $level; $i++) {
        if ($i == 0) {
            $string .= '<select class="pc-select-' . $id . '" name="' . $id . '-' . $i . '" id="' . $id . '-' . $i . '" width="100">
					<option value="">请选择</option>
    				<option value="3360;福州市" selected>福州市</option>
    				</select>';
        } else {
            $string .= '<select class="pc-select-' . $id . '" name="' . $id . '-' . $i . '" id="' . $id . '-' . $i . '" width="100">
					<option value="">请选择</option>';
            $string .= get_address_select_option($i, $areaid_array[$i - 1], $areaid_array[$i]);
            $string .= '</select> ';
        }
    }
    ...
    for ($i = 0; $i <= $level; $i++) {
        $next = $i + 1;
        $string .= '$("#' . $id . '-' . $i . '").change(function(){
						var slt_val1 = $("#' . $id . '-' . $i . '>option:selected").text();
						var slt_val2 = this.value;
						var slt_val = slt_val2+";"+slt_val1;
						$("#' . $id . '").val(slt_val);';
        for ($j = $i + 1; $j <= $level; $j++) {
            $string .= '$("#' . $id . '-' . $j . '").html("<option>请选择</option>");';
        }
        $string .= 'if(slt_val2!="" && slt_val2!="请选择" ){
				if(' . $i . '== 0){
					slt_val2 = 0;
				}';
        if ($i < $level) {
            $string .= '	$.get("' . APP_PATH . 'api.php?op=get_address_select_option",
								{parentid : slt_val2 },
								function(data){
        ...
        if ($i > 0) {
            $string .='else {
                var selectedObj = $("#' . $id . '-' . ($i - 1) . ' option:selected");
                //获取当前selected的值
                var selected = selectedObj.get(0).value;
                var selected_value = selectedObj.text();
                $("#' . $id . '").val(selected+";"+selected_value);
                }';
        }
        $string .= '});';
    }
...
function get_address($areaid, $id = 'villagename', $linkageid = 3360, $level = 3,$isParent=0) {
    $linkageid = intval($linkageid);
    $datas = array();
    $datas = getcache($linkageid, 'linkage');
    $infos = $datas['data'];
    ...
    $areaid_array = array_reverse($areaid_array); //逆序
    $string = '<input type="hidden" name="' . $id . '"  id="' . $id . '" value="' . $areaid . '">';
    ...
    for ($i = 0; $i <= $level; $i++) {
        if($i==0 )
        {
            if($isParent==1)
            {
                $string .= '<select class="pc-select-' . $id . '" name="' . $id . '-' . $i . '" id="' . $id . '-' . $i . '" width="100">
                    <option value="">请选择</option>
                    <option value="3360" '.$select_str.'>福州市</option>
                    </select> ';
            }
        }
        else
    ...
    for ($i=1; $i <= $level; $i++) {
        $next = $i + 1;
        $string .= '$("#' . $id . '-' . $i . '").change(function(){
						var slt_val = this.value;
						$("#' . $id . '").val(slt_val);';
        for ($j = $i + 1; $j <= $level; $j++) {
            $string .= '$("#' . $id . '-' . $j . '").html("<option>请选择</option>");';
        }
        $string .= 'if(slt_val!="" && slt_val!="请选择" ){';
        if ($i < $level) {
            $string .= '	$.get("' . APP_PATH . 'api.php?op=get_address_select_option",
								{parentid : slt_val },
								function(data){
 ...
 * 联动菜单层级: 返回id数组
 */
function menu_linkage_level_linkageid($linkageid, $infos, $result = array()) {
    if (empty($result)) {
        $result[] = $linkageid;
    }
    if (array_key_exists($linkageid, $infos)) {
        if ($infos[$linkageid]['parentid'])
            $result[] = $infos[$linkageid]['parentid'];
        return menu_linkage_level_linkageid($infos[$linkageid]['parentid'], $infos, $result);
    }
    return $result;
}
...
function get_address_select_option($i, $parentid, $selectedid, $keyid = 3360) {
    ...
    $parentid = $parentid == 1 ? 0 : $parentid; //一级联动菜单parentid=0
 ...
 * @param int $ms 跳转等待时间
 */
function showmessage($msg, $url_forward = 'goback', $ms = 1250, $dialog = '', $returnjs = '', $flag = '') {
    if (defined('IN_ADMIN')) {
 ...
 * @return 分页
 */
/* function pages($num, $curr_page, $perpage, $urlrule = '', $array = array(), $setpages = 10)
  {
  $perpage = $perpage ? $perpage : (PAGESIZE ? PAGESIZE : 20);
...
function pages($num, $curr_page, $perpage, $urlrule = '', $array = array(), $setpages = 10) {
    ...
    if ($num > $perpage) {
        ...
        $multipage .= '<div class="pageinfo">显示' . ($perpage * ($curr_page - 1) + 1) . '至' . ($perpage * ($curr_page)) . '/共' . $num . '条</div>';
        $multipage .= '<ul class="pagination">';
        if ($curr_page > 0) {
            $multipage .= ' <li class="prev"><a href="' . pageurl($urlrule, $curr_page - 1, $array) . '"><i class="double-angle-left-icon"></i></a></li>';
            if ($curr_page == 1) {
                $multipage .= '<li class="active"><a href="#">1</a></li>';
            } elseif ($curr_page > 6 && $more) {
                $multipage .= '<li><a href="' . pageurl($urlrule, 1, $array) . '">1</a></li><li><a href="javascript:;">...</a></li>';
            } else {
        ...
        if ($curr_page < $pages) {
            if ($curr_page < $pages - 5 && $more) {
                $multipage .= ' <li><a href="javascript:;">...</a></li><li><a href="' . pageurl($urlrule, $pages, $array) . '">' . $pages . '</a></li><li class="next"><a href="' . pageurl($urlrule, $curr_page + 1, $array) . '"><i class="double-angle-right-icon"></i></a></li>';
            } else {
                $multipage .= ' <li><a href="' . pageurl($urlrule, $pages, $array) . '">' . $pages . '</a></li><li class="next"><a href="' . pageurl($urlrule, $curr_page + 1, $array) . '"><i class="double-angle-right-icon"></i></a></li>';
            }
        } elseif ($curr_page == $pages) {
            $multipage .= ' <li class="active"><a href="#">' . $pages . '</a></li><li class="next"><a href="' . pageurl($urlrule, $curr_page, $array) . '"><i class="double-angle-right-icon"></i></a></li>';
        } else {
            $multipage .= ' <li><a href="' . pageurl($urlrule, $pages, $array) . '">' . $pages . '</a></li><li class="next"><a href="' . pageurl($urlrule, $curr_page + 1, $array) . '"><i class="double-angle-right-icon"></i></a></li>';
        }
        $multipage .= '</ul>';
