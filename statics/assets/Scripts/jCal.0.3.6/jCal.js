...
(function($) {
	...
	$.jCal = function (target, opt) {
	    opt = $.extend({
	        isMultiSelect: false,//modify by PSHQ
	...
	function drawCal (target, opt) {
		...
		$(target).find('div[id^=' + opt.cID + 'd_]:not(.invday)').bind("mouseover mouseout click", $.extend( {}, opt ),
			function(e){
					...
					if (e.type == 'click') {
					    if (!opt.isMultiSelect) {//modify by PSHQ
					        $('div[id*=d_]', e.data._target).stop().removeClass('selectedDay').removeClass('overDay').css('backgroundColor', '');
					    } else {
					        var $me = $(this);
					        if ($me.hasClass('selectedDay')) {
					            $me.removeClass('selectedDay');
					        } else {
					            $me.addClass('selectedDay');
					        }
					    }
					}
